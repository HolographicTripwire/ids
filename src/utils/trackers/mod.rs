...
//pub use sparse::SparseIdTracker;
...
struct IdTracker<I: Identifier,T: IdentifiedBy<I>, M: IntMap<Arc<Mutex<T>>>> {
    p: PhantomData<(I,T)>,
    map: M
}

impl <I: Identifier, T: IdentifiedBy<I>, M: IntMap<Arc<Mutex<T>>>> IdTracker<I,T,M> {
    /**
    ...
    pub fn put(&mut self, element: T) -> Arc<Mutex<T>> {
